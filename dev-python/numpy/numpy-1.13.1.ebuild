...
PATCHES=(
	"${FILESDIR}"/${PN}-1.13.1-no-hardcode-blas.patch
)
...
python_prepare_all() {
	if use lapack; then
        ...
        cat >> site.cfg <<-EOF || die
			[blas]
			include_dirs = $(pc_incdir cblas-int64)
			library_dirs = $(pc_libdir cblas-int64 blas-int64):${libdir}
			blas_libs = $(pc_libs cblas-int64 blas-int64)
			extra_compile_args = "$($(tc-getPKG_CONFIG) --cflags-only-other cblas-int64)"
			[lapack]
			library_dirs = $(pc_libdir lapack-int64):${libdir}
			lapack_libs = $(pc_libs lapack-int64)
			extra_compile_args = "$($(tc-getPKG_CONFIG) --cflags-only-other lapack-int64)"
		EOF
        else
        cat >> site.cfg <<-EOF || die
			[blas]
        ...
        if use int64; then
            ...
            append-cflags "$($(tc-getPKG_CONFIG) --cflags-only-other cblas-int64 lapack-int64)"
            append-ldflags "$($(tc-getPKG_CONFIG) --libs-only-other cblas-int64 lapack-int64)"
        else
            append-cflags "$($(tc-getPKG_CONFIG) --cflags-only-other cblas lapack)"
